...
* Cuenta con un panel simulador integrado en el cliente (accesible al pasar el cursor sobre las tarjetas de partidos y hacer clic en el botón de edición ⚙️) que te permite modificar marcadores, marcar partidos como "en vivo", finalizar encuentros y guardar estas anulaciones. Las tablas de posiciones y el feed del calendario se actualizarán al instante.

---

## 🚀 Estructura de la Suscripción (Parámetros del Calendario)

La ruta de API [/api/calendar](file:///d:/Proyectos/ANTIGRAVITY/calendario_mundial_2026/src/app/api/calendar/route.ts) genera dinámicamente el archivo del calendario según las preferencias seleccionadas por el usuario:

| Parámetro | Tipo | Descripción | Ejemplo |
| :--- | :--- | :--- | :--- |
| `team` | `string` | Filtra el calendario para obtener solo los partidos de un país. | `/api/calendar?team=Mexico` |
| `group` | `string` | Filtra los partidos de un grupo específico (A-L). | `/api/calendar?group=A` |
| `stadium` | `string` | Filtra por estadio (ID del estadio). | `/api/calendar?stadium=estadio-azteca` |
| `alarm` | `string` | Activa (`true`) o desactiva (`false`) recordatorios de eventos. | `/api/calendar?alarm=true` |
| `alarmMin`| `number` | Minutos de anticipación para el recordatorio (por defecto 15). | `/api/calendar?alarmMin=30` |
